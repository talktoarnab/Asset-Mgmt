...
def list_assets(org_id: str) -> list:
    ...
    return sorted(items, key=lambda a: a.get("title", ""))
...
def update_asset(org_id: str, asset_id: str, patch: dict) -> dict:
    ...
    if patch.get("status") and current.get("status") == "checked_out" and patch["status"] != "checked_out":
        raise conflict(
            "ASSET_ON_LOAN",
            f'"{current["title"]}" is on loan. Check it in (or mark the loan lost) to change its status.',
        )
...
def delete_asset(org_id: str, asset_id: str) -> None:
    ...
    if asset.get("status") == "checked_out":
        raise conflict("ASSET_ON_LOAN", f'"{asset["title"]}" is on loan and cannot be deleted.')
